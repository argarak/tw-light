...
am__installdirs = "$(DESTDIR)$(appicondir)" "$(DESTDIR)$(desktopdir)" \
	"$(DESTDIR)$(gamedatadir)"
DATA = $(appicon_DATA) $(desktop_DATA) $(gamedata_DATA)
...
STRIP = strip
VERSION = 0.5
abs_builddir = /home/kzer-za/Downloads/tw-light
abs_srcdir = /home/kzer-za/Downloads/tw-light
...
data/readme.html \
data/scp.ini \
data/scpgui/logo.bmp \
data/scpgui/menuaccept.wav \
data/scpgui/menudisabled.wav \
data/scpgui/menufocus.wav \
data/scpgui/menuspecial.wav \
data/scpgui/scptitle.bmp \
data/scpgui/titlemusic.wav \
data/server.ini \
data/shpaktgu.dat \
data/shpaktgu.ini \
data/shpalabc.dat \
...
install-gamedataDATA: $(gamedata_DATA)
	@$(NORMAL_INSTALL)
	@list='$(gamedata_DATA)'; test -n "$(gamedatadir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(gamedatadir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(gamedatadir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(gamedatadir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(gamedatadir)" || exit $$?; \
	done

uninstall-gamedataDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(gamedata_DATA)'; test -n "$(gamedatadir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(gamedatadir)'; $(am__uninstall_files_from_dir)
...
installdirs-am:
	for dir in "$(DESTDIR)$(appicondir)" "$(DESTDIR)$(desktopdir)" "$(DESTDIR)$(gamedatadir)"; do \
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am \
	...
	info-am install install-am install-appiconDATA install-data \
	install-data-am install-desktopDATA install-dvi install-dvi-am \
	install-exec install-exec-am install-gamedataDATA install-html \
	...
	uninstall-appiconDATA uninstall-desktopDATA \
	uninstall-gamedataDATA
